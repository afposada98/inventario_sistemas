...
//$pass = $select['pass'];
$pwd = $select['passwd'];		// USUARIO
$lgn = $select['login'];			// CONTRASEÑA
$id = $select['id'];			// ID USUARIO
$cedula = $select['cedula'];	// CEDULA USUARIO



$_SESSION['RUTA_INVENTARIO'] = $_SERVER["DOCUMENT_ROOT"] . '/Inventario';


//ESTA CONDICION VA A VER SI LOS DACTOS QUE LA PERSONA INGRESO SON LOS MISMOS QUE ESTAN EN LA BASE DE DATOS
if(($lgn == $login)&&($pwd == md5($password))) {
	
	$sql = "SELECT * FROM permisos WHERE id = '$id' and cod_programa = 6";
	...
	include("../base_datos/conexion_talento.php");
		...
		if($tipo==1) {
			...
			if($tipo==3) {
				header("Location:../inicio.php"); // admin
			} else {
				if($tipo==2) {
